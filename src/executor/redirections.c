...
/*   Updated: 2025/06/03 22:22:26 by my42             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int process_redirections(t_cmd *cmd)
{
    ...
    {
        // Process command-level heredoc if it exists
        ...
        // Process heredocs in redirection list
        if (cmd->redirections)
        {
...
int apply_redirections(t_cmd *cmd)
{
    ...
    // First check for cmd->input_fd (from heredoc processing)
    if (cmd->input_fd != -1)
    {
        printf("DEBUG: Command has direct input_fd=%d\n", cmd->input_fd);
        last_input_fd = cmd->input_fd;
    }
    
    if (!cmd->redirections)
    {
        // Only apply direct input_fd if it exists and no redirections override it
        if (last_input_fd != -1)
        {
            printf("DEBUG: Applying direct input_fd=%d\n", last_input_fd);
    ...
    // Process all redirections and track the last input/output fd
