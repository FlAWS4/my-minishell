...
/*   Updated: 2025/06/11 02:13:53 by mshariar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
                             ...
                             int prev_pipe, int *pipe_fds)
{
    ...
    // Make sure we have a clean output state
    fflush(stdout);
    fflush(stderr);
...
{
    // Only print newline for SIGINT and only in non-piped context
    if (signal_num == SIGINT && isatty(STDERR_FILENO) && isatty(STDOUT_FILENO))
        write(STDERR_FILENO, "\n", 1);
}
